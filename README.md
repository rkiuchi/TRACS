...
- ###### Note that the above "DoTracsOnly" executable requests the form of DoTracsOnly "number of threads" "config file".
However, number of threads == 1, is assumed for this version, ( the original code as well ) and no gurantee
if we try to run with more than one thread.
